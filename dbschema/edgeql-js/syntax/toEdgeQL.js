    ...
    for (const [expr, refData] of seen) {
        ...
        if (refData.refCount > 1 ||
            ...
            if (!withBlock) {
                ...
                const resolvedParentScopes = parentScopes.map(parentScope => withVars.get(parentScope)?.scope ?? parentScope);
                withBlock =
...
function renderEdgeQL(_expr, ctx, renderShape = true, noImplicitDetached = false) {
    ...
    if (withVar && ctx.renderWithVar !== expr) {
        return renderShape &&
            ...
            ? `(${withVar.name} ${shapeToEdgeQL((expr.__element__.__shape__ || {}), ctx, null, true // render shape only
            )})`
            : withVar.name;
    }
